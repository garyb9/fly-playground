...
| # | item | spec | status |
|---|------|------|--------|
| A1 | `palette.ts` retokenised to §2; existing key names kept where they map (`bg→void`, `pointCold→neuron`, `pointHot→spark`, `edge→pathway`, fly pair→ember) | §2, §6.1 | `todo` |
| A2 | `CONFIG.aesthetic` gains the motion + post-FX dials (`BREATH_*`, `BOB_*`, `ESCAPE_KICK`, `BLOOM_*`, `VIGNETTE`, `GRAIN`, `EXPOSURE`) | §5, §6.2 | `todo` |
| A3 | `EffectComposer` post-FX stack in the render path | §6.2 | `todo` |
...
| A12 | final HUD contrast re-checked against the real `void` background | §2.5 | `todo` |
